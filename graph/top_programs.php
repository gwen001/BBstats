...
$limit = 10;
$t_top = Statistics::top_program( $db );
//var_dump( $t_top );
...
<div class="row">
	<div class="col-md-12 text-center">
		<h4>Top <?php echo $limit; ?> Programs</h4>
	</div>
</div>
<div class="row">
	<div class="col-md-4 datop">
		<table class="table">
			...
			<tbody>
				<?php for( $i=1; $i<=$limit && list($program,$data)=each($t_top['t_n_report']) ; $i++ ) { ?>
					<tr class="top_<?php echo $i; ?>">
						<td><?php echo $i; ?></td>
						<td><span class="search-term"><?php echo ucwords($program); ?></span></td>
	...
	<div class="col-md-4 datop">
		<table class="table">
			...
			<tbody>
				<?php for( $i=1; $i<=$limit && list($program,$data)=each($t_top['t_bounty']) ; $i++ ) { ?>
					<tr class="top_<?php echo $i; ?>">
						<td><?php echo $i; ?></td>
						<td><span class="search-term"><?php echo ucwords($program); ?></span></td>
	...
	<div class="col-md-4 datop">
		<table class="table">
			...
			<tbody>
				<?php for( $i=1; $i<=$limit && list($program,$data)=each($t_top['t_reputation']) ; $i++ ) { ?>
					<tr class="top_<?php echo $i; ?>">
						<td><?php echo $i; ?></td>
						<td><span class="search-term"><?php echo ucwords($program); ?></span></td>
